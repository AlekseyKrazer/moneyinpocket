...
class MenuWidget extends Widget
{
    ...
    public function run()
    {
        ...
        switch ($this->source) {
            case 'income':
                $this->data = Categories::find()->indexBy("id")->asArray()->where(['type' => 2, 'user_id' => 1])->all();
                break;
            case 'groups':
            ...
            case 'deposit_exchange':
                break;
            case 'total-deposit':
                $this->data = Yii::$app->db->createCommand("Select * from (SELECT  CONCAT('dep_',id) as id, `name`, `group_id` as parent_id, images, 'dep' as type from deposits WHERE debt=0
                                                  UNION
...
(Select SUM(total) as total, deposit as deposit_id from (Select -1*SUM(amount) as total, deposit_from as deposit from exchange WHERE user_id=1 GROUP by deposit
 UNION
 Select SUM(amount) as total, deposit_to as deposit from exchange WHERE user_id=1 GROUP by deposit) as a JOIN deposits d ON a.deposit=d.id GROUP by deposit_id
 UNION
 Select total, dep.id as deposit_id from deposits dep JOIN (Select SUM(amount) as total, deposit_id from operations WHERE user_id=1 GROUP by deposit_id) as dd ON dd.deposit_id=dep.id
 UNION
 SELECT start_sum AS total, id AS deposit_id FROM deposits
 ) AS a 
 GROUP by deposit_id) AS two ON one.id=two.deposit_id")->queryAll();
...
Select * from 
  ...
  (Select SUM(total) as total,CONCAT('dep_',deposit_id) as deposit_id from 
      ...
      FROM (
            ...
            FROM exchange WHERE user_id=1 GROUP by deposit
            UNION
          ...
          WHERE user_id=1 GROUP by deposit_id
          ) as dd ON dd.deposit_id=dep.id WHERE dep.debt=1
          UNION
          SELECT start_sum as total, id as deposit_id from deposits
      ) as b 
        GROUP by deposit_id) as two ON one.id=two.deposit_id WHERE total>0 or total IS NULL")->queryAll();
                foreach ($this->data as $k) {
                    $total=$k['total']+$total;
                }
                $this->data=ArrayHelper::index($this->data,"id");
      ...
      FROM (
        ...
        GROUP by deposit_id) as two ON one.id=two.deposit_id WHERE total<0 OR total is NULL")->queryAll();
            ...
            case 'groups-debt':
                ...
                break;
            default:
                $this->data = Categories::find()->indexBy("id")->asArray()->where(['type' => 1, 'user_id' => 1])->all();
