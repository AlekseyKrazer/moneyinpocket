...
class AccountingController extends Controller
{
    ...
    public function actionTest($type,$date=false)
    {
        ...
        if ($type==3) {
            $data_for = Exchange::find()
                ...
                ->where(['date' => $date, 'exchange.user_id' => $user_id])
                ->orderBy("datetime")
                ->asArray()->all();
        } else {
    ...
    public function actionIndex($id=false, $date=false)
    {
        ...
        if ($type == 3) {
            ...
            if (isset($id) and $id!='') {
                $operations = Exchange::find()->where(['id' => $id, 'user_id' => 1])->one();
            }
        } else {
                $operations = new Operations();
            if (isset($id) and $id!='') {
                $operations = Operations::find()->where(['id' => $id, 'user_id' => 1])->one();
            }
        }
        ...
        if ($operations->load($add_array)) {
            ...
            if ($operations->save()) {
                    ...
                    $this->YiiSetDataCookie($add_array[$operations->formName()]['datetime']);
                    if (isset($add_array[$operations->formName()]['deposit_id'])) {
                        $this->YiiSetSelectCookie("deposit", $add_array[$operations->formName()]['deposit_id']);
                    }
        ...
        $dep = Yii::$app->db->createCommand('SELECT  CONCAT(\'dep_\',id) as id, `name`, `group_id` as parent_id, images, \'dep\' as type from deposits WHERE debt=0
                                                  UNION
                                                  SELECT id, name, parent_id, \'\' as images, \'cat\' as type from groups WHERE debt=0')->queryAll();
        $dep=ArrayHelper::index($dep,"id");

        $debts = Yii::$app->db->createCommand('SELECT  CONCAT(\'dep_\',id) as id, `name`, `group_id` as parent_id, images, \'dep\' as type from deposits WHERE debt=1
                                                  UNION
                                                  SELECT id, name, parent_id, \'\' as images, \'cat\' as type from groups WHERE debt=1')->queryAll();
        ...
        if ($operations->datetime!='') {
            $this->YiiSetDataCookie($operations->datetime);
        ...
        if ($type == 3 ) {
            ...
            $operations_data = Yii::$app->db->createCommand("
              ...
              JOIN categories cat  ON cat.id=op.category_id and cat.user_id=op.user_id
              JOIN deposits dep ON op.deposit_id=dep.id and dep.user_id=op.user_id
              WHERE op.user_id=" . $user_id . " and date='" . $date . "' and op.type=" . $type . " ORDER by datetime")->queryAll();
        }
    ...
    public function actionDelete($id, $type) {
        //Здесь надо будет вставить проверку на залогиненного бзера
        $user_id = 1;

        if ($type==3) {
            $model = Exchange::find()->where(['id' => $id, 'user_id' => $user_id])->one();
        } else {
            $model = Operations::find()->where(['id' => $id, 'user_id' => $user_id])->one();
        }
        if (empty($model)) {
    ...
    public function actionCategoryUpdate($id)
    {
        ...
        $model = Categories::find()->where(['id' => $id, 'user_id' => $user_id])->one();
        if (empty($model)) {
    ...
    public function actionCategoryDelete($id)
    {
        //Здесь надо будет вставить проверку на залогиненного бзера
        $user_id = 1;
        $count = Operations::find()->where(['category_id' => $id, 'user_id' => $user_id])->count();
    ...
    public function actionDepositUpdate($id)
    {
        ...
        $deposit = Deposits::find()->where(['id' => $id, 'user_id' => $user_id])->one();

        $count = Operations::find()->where(['deposit_id' => $id, 'user_id' => $user_id])->count();
    ...
    public function actionDepositDelete($id)
    {
        ...
        if ($count==0 and $count_exchange==0) {
            $model = Deposits::find()->where(['id' => $id, 'user_id' => $user_id, 'debt' => 0])->one();
            if (empty($model)) {
    ...
    public function actionDebt($id = false)
    {
        ...
        $dep = Yii::$app->db->createCommand('SELECT  CONCAT(\'dep_\',id) as id, `name`, `group_id` as parent_id, images, \'dep\' as type from deposits WHERE debt=1
                                                  UNION
                                                  SELECT id, name, parent_id, \'\' as images, \'cat\' as type from groups WHERE debt=1')->queryAll();
    ...
    public function actionDebtDelete($id)
    {
        ...
        if ($count==0 and $count_exchange==0) {
            $model = Deposits::find()->where(['id' => $id, 'user_id' => $user_id, 'debt' => 1])->one();
            if (empty($model)) {
