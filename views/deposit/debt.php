...
<div class="col-md-4">
    ...
    echo MenuWidget::widget(['tpl' => 'menu_comb','source' => 'debt', 'data'=> $dep]);


    ?>
</div>
<div class="col-md-4">
    <h3>Создание долга/займа</h3>
    ...
    <select id="income-parent_id" class="form-control" name="<?= $deposit->formName() ?>[group_id]"
        ...
        <?= MenuWidget::widget(['tpl' => 'select', 'model' => $deposit, 'source' => 'groups-debt']) ?>
    </select>
    ...
    <?= Html::submitButton('Создать долг') ?>
    <?php ActiveForm::end() ?>
    <br>
    ...
    <select id="income-parent_id" class="form-control" name="<?= $groups->formName() ?>[parent_id]"
            aria-invalid="false">
        <option value="0">-=пусто=-</option>
        <?= MenuWidget::widget(['tpl' => 'select', 'model' => $groups, 'source' => 'groups-debt']) ?>
    </select>
    ...
    <?= Html::submitButton('Создать группу долгов') ?>
    <?php ActiveForm::end() ?>
    <br>
    <?php if (in_array(Yii::$app->controller->module->requestedRoute, ['accounting/debt-update'])): ?>
        <h3>Перенос</h3>
        ...
        <select id="operations-deposit_id" class="form-control" name="Transfer[deposit_from]" aria-required="true">
            <?= MenuWidget::widget(['tpl' => 'select_comb','source' => 'debt', 'data'=> $dep, 'model' => $deposit]); ?>
        </select><BR>
        ...
        <select id="operations-deposit_id" class="form-control" name="Transfer[deposit_to]" aria-required="true">
            <?= MenuWidget::widget(['tpl' => 'select_comb','source' => 'debt', 'data'=> $dep]); ?>
        </select><BR>
