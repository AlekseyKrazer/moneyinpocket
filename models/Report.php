    ...
    public static function getDataChart( $dates, $type ) {
        ...
        $sql_data = Yii::$app->db->createCommand(
            "
              ...
              WHERE op.user_id=" . Yii::$app->user->id . " and date>='" . $dates_arr[0] . "' and date<='" . $dates_arr[1] . "' and op.type=" . $type . " GROUP by category_name ORDER by amount ASC"
        )->queryAll();
    ...
    public static function getTotalMoney( $type ) {
        $sql_data = Yii::$app->db->createCommand(
            "SELECT SUM(amount) as amount 
              FROM operations 
              WHERE op.user_id=" . Yii::$app->user->id . " and date>='2018-12-01' and op.type=" . $type . " GROUP by category_name ORDER by amount ASC"
        )->queryAll();
